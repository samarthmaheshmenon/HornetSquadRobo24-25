...
public class ManualManager {
    ...
    private AutoArmManager autoArmManager;

    private AutoViperSlideManager autoViperSlideManager;

    private AutoGrabberManager autoGrabberManager;

    private AutoDriveManager autoDriveManager;
    ...
    public ManualManager(LinearOpMode opmode, HornetRobo hornetRoboObj)  {
        ...
        autoArmManager = new AutoArmManager(opmode, hornetRobo);

        autoDriveManager = new AutoDriveManager(opmode, hornetRobo);

        autoGrabberManager = new AutoGrabberManager(opmode, hornetRobo);

        autoViperSlideManager = new AutoViperSlideManager(opmode, hornetRobo);
    }
    public void Init()    {
        //Mapping hardware devices
        AutoHardwareMapper.MapToHardware(myOpMode, hornetRobo);
        autoArmManager.SetDirection(AutoDriveManager.DriveDirection.FORWARD);
    ...
    public double GetLeftMotorRotations(){
        return autoDriveManager.GetCurrentPosition(AutoDriveManager.MotorPosition.LEFT_FRONT) /ticksPerRotationOfLeft;
    }
    public double GetRightMotorRotations(){
        return autoDriveManager.GetCurrentPosition(AutoDriveManager.MotorPosition.RIGHT_FRONT) / ticksPerRotationOfRight;
    }

    public void SetLeftTargetPosition(int leftTargetPosition){
        autoDriveManager.SetTargetPosition(AutoDriveManager.DriveDirection.LEFT, leftTargetPosition);

    }

    public void SetRightTargetPosition(int rightTargetPosition){
        autoDriveManager.SetTargetPosition(AutoDriveManager.DriveDirection.RIGHT, rightTargetPosition);
    }

    public boolean IsLeftMotorBusy()
    {
        return autoDriveManager.IsMotorBusy(AutoDriveManager.MotorPosition.LEFT_FRONT);
    }

    public boolean isRightMotorBusy()
    {
        return autoDriveManager.IsMotorBusy(AutoDriveManager.MotorPosition.RIGHT_FRONT);
    }

    public void SetMotorDirection(AutoDriveManager.DriveDirection Direction){
       autoDriveManager.SetMotorDirection(Direction);
    }

    public void SetViperSlideDirectionForward() {
        autoViperSlideManager.SetDirection(AutoDriveManager.DriveDirection.FORWARD);
    }

    public void SetViperSlideDirectionReverse() {
        autoViperSlideManager.SetDirection(AutoDriveManager.DriveDirection.BACKWARD);
    }

    public void ReverseMotors(){
        autoDriveManager.SetMotorDirection(AutoDriveManager.DriveDirection.BACKWARD);
    }
